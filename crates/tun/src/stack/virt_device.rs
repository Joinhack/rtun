...
use crate::stack::StackPacket;

pub struct VirtTunDevice {
    capabilities: DeviceCapabilities,
    in_buf: mpsc::UnboundedReceiver<StackPacket>,
    out_buf: mpsc::Sender<StackPacket>,
...
impl VirtTunDevice {
    ...
    ) -> (Self, mpsc::UnboundedSender<StackPacket>, Arc<AtomicBool>) {
...
impl Device for VirtTunDevice {
    ...
    fn receive(&mut self, _timestamp: Instant) -> Option<(Self::RxToken<'_>, Self::TxToken<'_>)> {
        if let Ok(buffer) = self.in_buf.try_recv() {
            ...
            let Ok(perm) = self.out_buf.try_reserve() else {
                self.in_buf_avail.store(false, Ordering::Release);
                return None;
            };
            let tx = VirtTxToken(perm);
            return Some((rx, tx));
        }
        self.in_buf_avail.store(false, Ordering::Release);
        None
...
pub struct VirtTxToken<'a>(Permit<'a, StackPacket>);
...
impl phy::TxToken for VirtTxToken<'_> {
    ...
    where
        F: FnOnce(&mut [u8]) -> R,
    {
        let mut buffer = StackPacket::with_capacity(len);
        unsafe {
            buffer.set_len(len);
        }
...
static TOKEN_BUFFER_LIST: LazyLock<Mutex<Vec<StackPacket>>> =
...
impl From<StackPacket> for TokenBuffer {
    #[inline(always)]
    fn from(buffer: StackPacket) -> Self {
        TokenBuffer { buffer }
    }
}
...
impl Drop for TokenBuffer {
    fn drop(&mut self) {
        ...
        let empty_buffer = StackPacket::new();
...
impl TokenBuffer {
    pub fn with_capacity(cap: usize) -> Self {
        ...
        Self {
            buffer: StackPacket::with_capacity(cap),
        }
    }
}

impl Deref for TokenBuffer {
    type Target = StackPacket;
